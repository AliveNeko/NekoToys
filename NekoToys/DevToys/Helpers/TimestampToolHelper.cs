...
namespace NekoToys.DevToys.Helpers;
...
internal static class TimestampToolHelper
{
    internal static class ZoneInfo
    {
        ...
        private static readonly Lazy<IReadOnlyDictionary<string, TimeZoneInfo>> LazyTimeZoneInfos = new(() => _systemTimeZone.ToDictionary(x => x.Id, x => x));
        internal static IReadOnlyDictionary<string, TimeZoneInfo> TimeZoneInfos => LazyTimeZoneInfos.Value;
        ...
        private static IReadOnlyDictionary<string, string> InitTimeZoneCollection()
        {
            ...
            if (!Regex.IsMatch(_systemTimeZone.ElementAt(0).DisplayName, @"^\(UTC.*\).+$"))
            {
                ...
                {
                    string displayName =
                        $"(UTC{zone.BaseUtcOffset.Hours:+00;-00;}:{zone.BaseUtcOffset.Minutes:00;00;}) " +
                        zone.DisplayName;
